...
function buildWcagTextReport(results: AxeResult): string {
  ...
  lines.push('Generated by Retro Web Editor')
  lines.push('='.repeat(60))
...
function updateWindowTitle(): void {
  ...
  if (tab) {
    ...
    window.api.setWindowTitle(`${tab.fileName}${dirty} - Retro Web Editor`)
  } else {
    window.api.setWindowTitle('Retro Web Editor')
  }
}
...
const HELP_DESCRIPTIONS: Record<string, [string, string]> = {
  ...
  'app-branding': ['Retro Web Editor', 'A lightweight WYSIWYG HTML editor. Go to Help > About for more info.'],
  ...
  'hotkeys-overlay': ['Keyboard Shortcuts', 'Reference of all keyboard shortcuts available in Retro Web Editor.'],
